...
public class Signal : IRenderable
{
    ...
    public void Render(IRenderContext ctx)
    {
        ...
        SignalRenderer.FillDecimatedPath(data.YValues, data.XRange, XAxis, YAxis, ctx.ClientRect.Size, path);
        //SignalRenderer.FillFullPath(data.YValues, data.XRange, XAxis, YAxis, ctx.ClientRect.Size, path);
        ctx.Canvas.DrawPath(path, Paint);
        //SignalRenderer.DrawVerticalLines(data.YValues, data.XRange, XAxis, YAxis, ctx.Canvas, Paint, ctx.ClientRect.Size);
        ctx.Canvas.Restore();
    }
}